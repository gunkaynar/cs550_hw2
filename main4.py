...
model = ANN(test_data_n, test_labels_n, input_size=1, hidden_size=8, output_size=1, learning_rate=0.0001,
            ...
            activation_function_back=tanh_back, momentum=0)
...
plt.legend()
plt.suptitle(f'Learning Curve without momentum Training Set 2')
# plt.savefig(f'learning_curve_nm_2.png')
plt.show()
plt.clf()

model = ANN(test_data_n, test_labels_n, input_size=1, hidden_size=8, output_size=1, learning_rate=0.0001,
            activation_function=tanh,
            activation_function_back=tanh_back, momentum=0.04)
model.train_epoch(train_data_n, train_labels_n, epochs=1000, batch_size=1)
...
plt.plot(model.loss_history_test, label='Test Loss')
plt.legend()
plt.suptitle(f'Learning Curve with momentum Training Set 2')
# plt.savefig(f'learning_curve_wm_2.png')
plt.show()
plt.clf()
